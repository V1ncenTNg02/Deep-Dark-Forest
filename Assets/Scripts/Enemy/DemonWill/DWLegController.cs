...
{
    public class DWLegController : MonoBehaviour
    {
        [Header("Animation")]
        ...
        [Tooltip("DWLeg placement radius offset")]
        ...
        // Make sure the variables are making sense
        ...
        {
            foreach (DWLeg g in GameObject.FindObjectsOfType<DWLeg>())
            {
        ...
        void Update()
        {
            ...
            if (legCount <= maxLegs - partsPerLeg)
            {
                ...
                RaycastHit hit;
                Physics.Raycast(newLegPosition + Vector3.up * 3f, Vector3.down, out hit);
                ...
                {
                    RequestLeg(myHit, legResolution, maxLegDistance, Random.Range(minGrowCoef, maxGrowCoef), lifeTime);
                    if (legCount >= maxLegs)
                        return;
                }
            }
        ...
        void RequestLeg(Vector3 footPosition, int legResolution, float maxLegDistance, float growCoef, float lifeTime)
        {
            GameObject newLeg;

            // Use NewLeg based on leg pool
            ...
            newLeg.GetComponent<DWLeg>().Initialize(footPosition, legResolution, maxLegDistance, growCoef, this, lifeTime);

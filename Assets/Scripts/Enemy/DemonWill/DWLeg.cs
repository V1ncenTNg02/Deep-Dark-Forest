/* Generate the Legs based on the LineRenderer.
 Detect possible foot points and decide the splines points to 
 create the vertexes of line. */
// This is the modified assets which originally come from unity asset store
...
{
    public class DWLeg : MonoBehaviour
    {
        ...
        public int handlesCount = 8; // 8 (7 legs + 1 final foot)
        ...
        public void Initialize(Vector3 footPosition, int legResolution, float maxLegDistance, float growCoef, DWLegController myDwLegController, float lifeTime)
        {
            this.footPosition = footPosition;
            this.legResolution = legResolution;
            this.maxLegDistance = maxLegDistance;
            this.growCoef = growCoef;
            this.myDwLegController = myDwLegController;
        ...
        IEnumerator WaitAndDie(float lifeTime)
        {
            ...
            while (myDwLegController.deployedLegs < myDwLegController.minimumAnchoredParts)
                yield return null;
            growTarget = 0;
        }
        ...
        private void Update()
        {
            ...
            if (Math.Abs(growTarget - 1) < 0.001f)
            {
                if (Vector3.Distance(new Vector3(myDwLegController.legPlacerOrigin.x, 0, myDwLegController.legPlacerOrigin.z),
                        new Vector3(footPosition.x, 0, footPosition.z)) > maxLegDistance && canDie &&
                    myDwLegController.deployedLegs > myDwLegController.minimumAnchoredParts)
                {
                    growTarget = 0;
                }

                // Check is the body is in line of sight from the foot position, and initiates the retractation if it isn't
                if (Physics.Linecast(footPosition, transform.position))
                {
                    growTarget = 0;
                }
            }
            ...
            // we signal the leg deployement to the DWLegController for the leg spawn logic
            if (!isDeployed && progression > 0.9f)
            {
                myDwLegController.deployedLegs++;
                isDeployed = true;
            }
            ...
            if (progression < 0.5f && growTarget == 0)
            {
                if (!isRemoved)
                {
                    GetComponentInParent<DWLegController>().legCount--;
                    isRemoved = true;
                }
                ...
                if (progression < 0.05f)
                {
                    ...
                    myDwLegController.RecycleLeg(this.gameObject);
                    return;
                }
